@echo off
echo Dang dung chatbot/backend/frontend dang chay (neu co)...
...
taskkill /FI "WINDOWTITLE eq Chatbot Server*" /T /F >nul 2>&1
taskkill /FI "WINDOWTITLE eq Backend Server*" /T /F >nul 2>&1
taskkill /FI "WINDOWTITLE eq Frontend Server*" /T /F >nul 2>&1

REM Du phong: neu cua so bi dong tay truoc do nhung tien trinh van con giu port -- tim theo port dang
REM chay that (3200/8080/5173) roi diet dung PID do, khong dua theo ten cua so nua.
for /f "tokens=5" %%P in ('netstat -ano ^| findstr ":3200 " ^| findstr LISTENING') do taskkill /PID %%P /F >nul 2>&1
...
REM Thu tu giong start.bat: chatbot truoc, vi backend goi sang kit qua http://localhost:3200.
